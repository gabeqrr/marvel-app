...
export class CharactersService {
  ...
  getCharacters(): Observable<Character> {
    console.log('OFFSET NO SERVICE', this.offSetForPagination);
    return this.http.get<Character>(
      `http://gateway.marvel.com/v1/public/characters?offset=${this.offSetForPagination}&limit=${this.limitOfCharacters}&ts=${this.time}&apikey=${this.publicKey}&hash=${this.hash}`
    );
  }
}
